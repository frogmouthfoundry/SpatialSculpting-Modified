...
// Multi-map payload used when loading sculpt packages.
struct SculptLoadPayload {
    ...
    let textureBoundsMin: SIMD3<Float>
    let textureBoundsMax: SIMD3<Float>
    let averageRoughness: Float
    let averageAlbedo: SIMD3<Float>
}
...
struct SculptingToolSystem: ComputeSystem {
    ...
    func update(computeContext: inout ComputeUpdateContext) {
        for sculptingTool in computeContext.sceneUpdateContext.scene.performQuery(query) {
            ...
            // Load a complete sculpt package payload.
            if let payload = sculptingToolComponent.loadPayload {
                sculptingToolComponent.sculptor.load(payload: payload, computeContext: &computeContext)
                sculptingToolComponent.loadPayload = nil
            }
