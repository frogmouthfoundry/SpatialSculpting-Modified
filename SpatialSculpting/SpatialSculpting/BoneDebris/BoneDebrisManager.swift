...
final class BoneDebrisManager {
    ...
    static let debrisStretch: Float = 3.5   // elongation along depth axis
    
    private let stepDistance: Float = 0.002
    private let minimumMovement: Float = 0.0005
    
    static let debrisGravity: SIMD3<Float> = SIMD3<Float>(2.5, 0, -4.5)
    ...
    var ejectionForceMultiplier: SIMD3<Float> = SIMD3<Float>(1.0, 1.0, 1.0)
    /// Frames to wait before applying the ejection impulse.
    private let ejectionDelayFrames: Int = 1
    ...
    var growthMaxMultiplier: SIMD3<Float> = SIMD3<Float>(6.0, 6.0, 2.0)
    ...
    private func placeBox(at position: SIMD3<Float>, direction: SIMD3<Float>) {
        ...
        // Option 4: Offset spawn position outward from the volume center (origin).
        // This ensures debris starts outside the collision mesh, not inside it.
        ...
        // Use capsule collision: closest primitive to a stretched sphere.
        let capsuleHeight = Self.debrisRadius * 2.0 * (Self.debrisStretch - 1.0)
        let debrisShape = ShapeResource.generateCapsule(
